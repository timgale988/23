...
export function PrysmSignalMapR3() {
  return (
    <section
    ...
    >
      ...
      >
        ...
        <div
          ...
          style={{ background: "radial-gradient(ellipse 80% 70% at 55% 45%, #F4F1FD 0%, #EEEEF4 40%, #EBEBEB 100%)" }}
        >
          {/* Soft white wash — keeps diagram readable without killing the color */}
          <div
          ...
          />
          {/* Edge vignette only — doesn't touch the center */}
          <div
            className="absolute inset-0 pointer-events-none"
            style={{
              background:
                "radial-gradient(ellipse 95% 90% at 52% 50%, transparent 45%, rgba(239,239,239,0.45) 78%, transparent 100%)",
            }}
          />
          {/* Subtle grid */}
          <div
          ...
          <div className="relative w-full z-10" style={{ height: "480px" }}>
            <svg viewBox="0 0 580 480" className="w-full h-full overflow-visible">
              <defs>
                ...
                {/* Add a clip path for the node borders to keep rounded corners */}
                <clipPath id="nodeClip">
                  <rect width="150" height="44" rx="8" />
                </clipPath>
              </defs>

              {/* Outer orbit rings */}
              <circle cx="290" cy="225" r="155" fill="none" stroke="#DDDDDD" strokeWidth="0.5" strokeDasharray="4 12" opacity="0.35" />

              {/* INPUT CONNECTOR LINES */}
              {[55, 145, 230, 315].map((y) => (
                <path
                  key={`in-${y}`}
                  d={`M 158 ${y + 22} C 230 ${y + 22}, 255 225, 290 225`}
                  fill="none"
                  stroke="#C0C0C0"
                  strokeWidth="1.8"
                />
              ...
              {[55, 145, 230, 315].map((y) => (
                <path
                  key={`out-${y}`}
                  d={`M 290 225 C 330 225, 360 ${y + 22}, 422 ${y + 22}`}
                  fill="none"
                  stroke="#C0BAE8"
                  strokeWidth="1.8"
              ...
              {/* CENTRAL HUB */}
              <circle cx="290" cy="225" r="85" fill="rgba(124,58,237,0.04)" />
              <circle cx="290" cy="225" r="83" fill="none" stroke="#7C3AED" strokeWidth="1" opacity="0.4" />
              <circle cx="290" cy="225" r="78" fill="#111111" filter="url(#smShadowMono)" />
              <circle cx="290" cy="225" r="78" fill="none" stroke="rgba(255,255,255,0.12)" strokeWidth="1.5" />
              <circle cx="290" cy="225" r="70" fill="none" stroke="rgba(255,255,255,0.05)" strokeWidth="1" />
              <circle cx="290" cy="225" r="60" fill="none" stroke="rgba(255,255,255,0.10)" strokeWidth="0.75" />
              <circle cx="290" cy="225" r="52" fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth="1" />

              <text x="290" y="218" textAnchor="middle" fill="#FFFFFF" fontSize="19" fontWeight="900" letterSpacing="4" fontFamily="Urbanist, sans-serif">PRYSM</text>
              <text x="290" y="238" textAnchor="middle" fill="rgba(255,255,255,0.45)" fontSize="10" fontWeight="600" letterSpacing="2.5" fontFamily="Urbanist, sans-serif">AI ENGINE</text>
              <text x="290" y="268" textAnchor="middle" fill="rgba(255,255,255,0.4)" fontSize="8" fontWeight="600" letterSpacing="0.2em" fontFamily="Urbanist, sans-serif">INTELLIGENT ROUTING</text>

              {/* INPUT NODES */}
              {[
                { y: 55,  label: "Epic EHR",       sub: "EMR Integration" },
                { y: 145, label: "Windfall",        sub: "Wealth Screening" },
                { y: 230, label: "DonorSearch",     sub: "Philanthropic Data" },
                { y: 315, label: "70K+ Physicians", sub: "Referral Network" },
              ].map(({ y, label, sub }) => (
                <g key={label} transform={`translate(8, ${y})`} filter="url(#smShadowMono)">
                  ...
                  <g clipPath="url(#nodeClip)">
                    <rect x="0" y="0" width="3" height="44" fill="#444444" />
                  </g>
                  <circle cx="17" cy="22" r="4" fill="#333333" />
                  <text x="30" y="16" fill="#111111" fontSize="12.5" fontWeight="800" fontFamily="Urbanist, sans-serif">{label}</text>
                  <text x="30" y="31" fill="#888888" fontSize="10" fontWeight="500" fontFamily="Urbanist, sans-serif">{sub}</text>
                </g>
              ))}

              {/* OUTPUT NODES */}
              {[
                { y: 55,  label: "Blackbaud CRM",   sub: "Gift Management" },
                { y: 145, label: "Salesforce NPSP", sub: "Fundraiser CRM" },
                { y: 230, label: "Gift Officers",   sub: "Prospect Routing" },
                { y: 315, label: "ROI Analytics",   sub: "Board Reporting" },
              ].map(({ y, label, sub }) => (
                <g key={label} transform={`translate(424, ${y})`} filter="url(#smShadowMono)">
                  <rect width="150" height="44" rx="8" fill="#FFFFFF" stroke="#DCDCDC" strokeWidth="1.5" />
                  <g clipPath="url(#nodeClip)">
                    <rect x="147" y="0" width="3" height="44" fill="#888888" />
                  </g>
                  <circle cx="133" cy="22" r="4" fill="#666666" />
                  <text x="118" y="16" textAnchor="end" fill="#111111" fontSize="12.5" fontWeight="800" fontFamily="Urbanist, sans-serif">{label}</text>
                  <text x="118" y="31" textAnchor="end" fill="#888888" fontSize="10" fontWeight="500" fontFamily="Urbanist, sans-serif">{sub}</text>
                </g>
              ))}
            </svg>
          </div>
          ...
          >
            {[
              { value: "12,847", label: "Encounters Processed", dot: "#111111" },
              { value: "1,422",  label: "Prospects Qualified",   dot: "#444444" },
              { value: "47",     label: "New Today",             dot: "#888888" },
            ].map(({ value, label, dot }) => (
              <div
                key={label}
                className="rounded-xl px-4 py-3 flex flex-col justify-center gap-0.5"
                style={{
                  background: "#FFFFFF",
              ...
              >
                <div className="text-[15px] font-black leading-tight" style={{ color: "#111111" }}>{value}</div>
