...
export function PrysmSignalMapR4() {
  return (
    <section
    ...
    >
      ...
      >
        ...
        <div
          ...
          style={{ background: "radial-gradient(ellipse 70% 60% at 55% 45%, #0F1228 0%, #0A0C18 100%)" }}
        >
          {/* Subtle grid */}
          <div
          ...
          <div className="relative w-full z-10" style={{ height: "480px" }}>
            <svg viewBox="0 0 580 480" className="w-full h-full overflow-visible">
              ...
              {/* Outer orbit rings */}
              <circle cx="290" cy="225" r="148" fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth="0.75" strokeDasharray="5 9" />
              <circle cx="290" cy="225" r="110" fill="none" stroke="rgba(255,255,255,0.04)" strokeWidth="0.5" strokeDasharray="3 7" />

              {/* INPUT CONNECTOR LINES */}
              {[55, 145, 230, 315].map((y) => (
                <path
                  key={`in-${y}`}
                  d={`M 158 ${y + 22} C 218 ${y + 22}, 255 225, 290 225`}
                  fill="none"
              ...
              {[55, 145, 230, 315].map((y) => (
                <path
                  key={`out-${y}`}
                  d={`M 290 225 C 340 225, 365 ${y + 22}, 424 ${y + 22}`}
                  fill="none"
              ...
              {/* CENTRAL HUB */}
              <circle cx="290" cy="225" r="90" fill="rgba(255,255,255,0.03)" />
              <circle cx="290" cy="225" r="84" fill="none" stroke="rgba(255,255,255,0.12)" strokeWidth="1" />
              <circle cx="290" cy="225" r="78" fill="#FFFFFF" filter="url(#smShadowMono)" />
              
              <text x="290" y="218" textAnchor="middle" fill="#0A0C18" fontSize="19" fontWeight="900" letterSpacing="4" fontFamily="Urbanist, sans-serif">PRYSM</text>
              <text x="290" y="238" textAnchor="middle" fill="#6B7280" fontSize="10" fontWeight="600" letterSpacing="2.5" fontFamily="Urbanist, sans-serif">AI ENGINE</text>
              <text x="290" y="268" textAnchor="middle" fill="#6B7280" fontSize="8" fontWeight="600" letterSpacing="0.2em" fontFamily="Urbanist, sans-serif">INTELLIGENT ROUTING</text>
              ...
              {[
                ...
                <g key={label} transform={`translate(8, ${y})`} filter="url(#smShadowMono)">
                  <rect width="150" height="44" rx="8" fill="#13172A" stroke="rgba(255,255,255,0.08)" strokeWidth="1.5" />
                  <circle cx="17" cy="22" r="4" fill="#6B7280" />
                  <text x="30" y="17" fill="#E5E7EB" fontSize="11.5" fontWeight="800" fontFamily="Urbanist, sans-serif">{label}</text>
                  <text x="30" y="30" fill="#6B7280" fontSize="9.5" fontWeight="500" fontFamily="Urbanist, sans-serif">{sub}</text>
                </g>
              ))}
              ...
              {[
                ...
                <g key={label} transform={`translate(424, ${y})`} filter="url(#smShadowMono)">
                  <rect width="150" height="44" rx="8" fill="#13172A" stroke="rgba(255,255,255,0.08)" strokeWidth="1.5" />
                  <circle cx="133" cy="22" r="4" fill="#6B7280" />
                  <text x="118" y="17" textAnchor="end" fill="#E5E7EB" fontSize="11.5" fontWeight="800" fontFamily="Urbanist, sans-serif">{label}</text>
                  <text x="118" y="30" textAnchor="end" fill="#6B7280" fontSize="9.5" fontWeight="500" fontFamily="Urbanist, sans-serif">{sub}</text>
                </g>
              ))}
            </svg>
          </div>
          ...
          >
            {[
              { value: "12,847", label: "Encounters Processed", dot: "#7C3AED" },
              { value: "1,422",  label: "Prospects Qualified",   dot: "#6366F1" },
              { value: "47",     label: "New Today",             dot: "#4B5563" },
            ].map(({ value, label, dot }) => (
              <div
                key={label}
              ...
              >
                <div className="text-[15px] font-black leading-tight" style={{ color: "#F9FAFB" }}>{value}</div>
                <div className="text-[10px] font-semibold uppercase tracking-wide" style={{ color: "#6B7280" }}>{label}</div>
              </div>
            ))}
          </div>
        </div>
